...
public class RCCP_Stability : RCCP_Component {
    ...
    private void UpdateESP() {
        ...
        if (underSteering) {
            ...
            if (frontAxle.isBrake) {
                ...
                frontAxle.leftWheelCollider.AddBrakeTorque((frontAxle.maxBrakeTorque * (ESPIntensity * .25f)) * Mathf.Clamp(-rearSlip, 0f, Mathf.Infinity));
                frontAxle.rightWheelCollider.AddBrakeTorque((frontAxle.maxBrakeTorque * (ESPIntensity * .25f)) * Mathf.Clamp(rearSlip, 0f, Mathf.Infinity));
        ...
        if (overSteering) {
            ...
            if (rearAxle.isBrake) {
                ...
                rearAxle.leftWheelCollider.AddBrakeTorque((rearAxle.maxBrakeTorque * (ESPIntensity * .25f)) * Mathf.Clamp(-frontSlip, 0f, Mathf.Infinity));
                rearAxle.rightWheelCollider.AddBrakeTorque((rearAxle.maxBrakeTorque * (ESPIntensity * .25f)) * Mathf.Clamp(frontSlip, 0f, Mathf.Infinity));
    ...
    private void TurnHelper() {
        ...
        steeringDirection.localRotation = Quaternion.Euler(0f, frontAxle.steerAngle, 0f);
